...
function getIframeAnchor() {
  return getPageType() === "item"
    ? jQuery("div.workshopItemPreviewArea")
    : jQuery("div.collectionControls");
}

function getButtonAnchor() {
  return getPageType() === "item"
    ? jQuery("#SubscribeItemBtn")
    : jQuery("div.subscribeCollection:contains('Subscribe')")
        .find("> span")
        .last().length
    ? jQuery("div.subscribeCollection:contains('Subscribe')")
        .find("> span")
        .last() // Has a subscribe collection box, use that.
    : jQuery("div.workshopItemDescriptionTitle:contains('Items')").find(
        "> span"
      ); // Use the defailt items count title.
}
...
function getPageType() {
  if (
    jQuery("div.game_area_purchase_game").length &&
    jQuery("span.subscribeText").length
  ) {
    return "item";
...
function onDownloadWorkshopItem() {
  ...
  getIframeAnchor().after(`<iframe
              ...
              src="//steamworkshopdownloader.io/extension/embedded/${new URLSearchParams(
...
function addButton() {
  const downloadButton =
    getPageType() === "item"
      ...
      </a><h3 style="margin: 2px; float: right;">请在此网页关闭IDM之类的第三方下载器</h3>`
      : `<a id="downloadWorkshopItemButton" class="btn_darkblue_white_innerfade btn_medium" style="height: 26px; float: right;" onclick="window.parent.postMessage(['steamdownloaderButtonPressed', true], '*')"> 
...
function doesButtonExist() {
  return !!document.querySelector("#downloadWorkshopItemButton");
}

function isWallpaperEngineWorkshop(){
  return document.querySelector(".apphub_AppName").textContent == "Wallpaper Engine：壁纸引擎"
}
...
(() => {
  const load = () => {
    if(!isWallpaperEngineWorkshop())return;
    sendIdMessage()
